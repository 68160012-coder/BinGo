document.addEventListener('DOMContentLoaded', () => {
    ...
    const WASTE_TYPE_LABELS = {
        'general': 'ขยะทั่วไป', 'recyclable': 'ขยะรีไซเคิล',
        'organic': 'ขยะอินทรีย์', 'hazardous': 'ขยะอันตราย'
    };
    const STATUS_LABELS = { 'new': 'ใหม่', 'in_progress': 'ดำเนินการ', 'done': 'เสร็จสิ้น' };

    function formatNumber(num) {
        if (num === null || num === undefined) return '-';
        return Number(num).toLocaleString('th-TH', { minimumFractionDigits: 1, maximumFractionDigits: 1 });
    }
    ...
    function initReportPage() {
        const els = {
            form: reportForm, submitBtn: document.getElementById('submit-btn'),
            successMsg: document.getElementById('form-success'), errorMsg: document.getElementById('form-error'),
            list: document.getElementById('reports-list'), loading: document.getElementById('reports-loading'),
            empty: document.getElementById('reports-empty'), filter: document.getElementById('filter-type')
        };
        ...
        els.filter.addEventListener('change', () => {
            ...
            els.loading.querySelector('span').textContent = 'กำลังกรอง...';
        ...
        els.form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (res.status === 201) {
                    els.form.reset();
                    els.successMsg.textContent = `บันทึกแล้ว #${data.id}`;
                    els.successMsg.classList.remove('hidden');
                    const currentFilter = els.filter.value;
                    if (currentFilter === '' || currentFilter === data.waste_type) {
                        prependCard(data, els.list);
                        els.empty.classList.add('hidden');
                        els.list.classList.remove('hidden');
                    }
                } else {
                    els.errorMsg.textContent = data.error || 'เกิดข้อผิดพลาด';
                    els.errorMsg.classList.remove('hidden');
                }
            } catch {
                els.errorMsg.textContent = 'ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ได้';
                els.errorMsg.classList.remove('hidden');
            } finally {
    ...
    async function loadReports(els) {
        try {
            ...
            els.loading.classList.add('hidden');
            if (reports.length === 0) {
                els.empty.querySelector('p').textContent = type ? 'ไม่พบรายการประเภทนี้' : 'ยังไม่มีรายการแจ้งขยะ';
                els.empty.classList.remove('hidden');
            } else {
                els.list.classList.remove('hidden');
                reports.forEach(r => prependCard(r, els.list));
            }
        } catch (err) {
            els.loading.querySelector('span').textContent = err.message || 'โหลดไม่สำเร็จ';
        }
    }
    ...
    function prependCard(report, container) {
        ...
        typeSpan.textContent = WASTE_TYPE_LABELS[report.waste_type] || report.waste_type;
